...
export const useRealPredictiveAnalytics = () => {
  ...
  const { data: predictions, isLoading: predictionsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const predictions: Prediction[] = predictionData.map(pred => {
  ...
  const { data: patterns, isLoading: patternsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [patternsResult, correlationsResult] = await Promise.all([
        ...
        supabase.rpc('calculate_correlation_scores', { target_user_id: user.id })
      ]);
      ...
      behavioralPatterns.forEach((pattern) => {
        ...
        patterns.push({
          pattern: pattern.pattern_type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
  ...
  const runAnalysisMutation = useMutation({
    mutationFn: async () => {
      ...
      const [insightsResult, predictionsResult] = await Promise.all([
        supabase.rpc('generate_behavior_insights', { target_user_id: user.id }),
        supabase.rpc('generate_predictive_insights', { target_user_id: user.id })
      ]);
