...
export const useRoutines = () => {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            setRoutines((prev) => [payload.new as Routine, ...prev]);
          } else if (payload.eventType === 'UPDATE') {
            setRoutines((prev) =>
              prev.map((routine) =>
                routine.id === payload.new.id ? (payload.new as Routine) : routine
              )
            );
          } else if (payload.eventType === 'DELETE') {
            setRoutines((prev) => prev.filter((routine) => routine.id !== payload.old.id));
          }
        }
      )
      .subscribe();
  ...
  const createRoutine = async (
    routineData: Omit<Routine, 'id' | 'user_id' | 'created_at' | 'updated_at'>,
    steps?: Omit<RoutineStep, 'id' | 'routine_id' | 'created_at' | 'is_completed'>[]
  ) => {
    try {
  ...
  const updateRoutineStep = async (stepId: string, updates: Partial<RoutineStep>) => {
    try {
      ...
      console.error('Error updating routine step:', err);
      return { data: null, error: err.message };
    }
  };
  ...
  const deleteRoutineStep = async (stepId: string) => {
    try {
      ...
      console.error('Error deleting routine step:', err);
      return { error: err.message };
    }
  };

  const getMorningRoutines = () => {
    return routines.filter((routine) => routine.routine_type === 'morning' && routine.is_active);
  };

  const getEveningRoutines = () => {
    return routines.filter((routine) => routine.routine_type === 'evening' && routine.is_active);
  };
